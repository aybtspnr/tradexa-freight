  ...
  async function loadReferrals(uid: string) {
    ...
    if (data && data.length > 0) {
      ...
      setStats({
        ...
        reward_earned: data.some((r: any) => r.status === "rewarded")
          ? "1 mês Pro"
          : data.length >= 3
            ? "1 mês Premium"
            : data.length >= 5
      ...
      const localReferrals: ReferralEntry[] = stored ? JSON.parse(stored) : [];
      setReferrals(localReferrals);
      setStats({
        total_referrals: localReferrals.length,
        converted_referrals: localReferrals.filter((r) =>
          ["converted", "rewarded"].includes(r.status)
        ).length,
        reward_earned: localReferrals.some((r) => r.status === "rewarded")
          ? "1 mês Pro"
          : localReferrals.length >= 3
            ? "1 mês Premium"
            : localReferrals.length >= 5
              ? "1 mês Enterprise"
              : "—",
      });
    }
                    ...
                    <div key={ref.id} className="flex items-center justify-between rounded-lg border border-gray-100 p-3 dark:border-gray-700">
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                          {ref.referred_name ?? ref.referred_email ?? "Novo usuário"}
                        </p>
