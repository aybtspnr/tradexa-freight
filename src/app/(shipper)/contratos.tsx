          ...
          {contract.carrier_name && (
            <div className="flex items-center gap-2 rounded-lg border border-gray-200 p-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{contract.carrier_name}</p>
              </div>
            </div>
          )}
...
export function Contratos() {
  ...
  async function loadContracts() {
    ...
    // Batch fetch carrier names
    const carrierIds = [...new Set(data.map((c) => c.carrier_id))];
    const { data: carriers } = await supabase
      .from("profiles")
      .select("id, name")
      .in("id", carrierIds);

    const carrierMap = new Map((carriers ?? []).map((p) => [p.id, p.name ?? "Transportadora"]));

    const enriched: Contract[] = data.map((c) => ({
      ...c,
      carrier_name: carrierMap.get(c.carrier_id) ?? "Transportadora",
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Gerencie seus contratos de frete fixo com transportadoras.
          </p>
        </div>
        <button
          onClick={() => setShowNew(true)}
          className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary-dark active:scale-[0.98] transition-all"
        >
      ...
      {!loading && contracts.length > 0 && (
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 col-span-2 sm:col-span-1">
            <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Valor Mensal Est.</p>
      ...
      {loading ? (
        ...
        <div className="mt-12 flex flex-col items-center rounded-2xl border-2 border-dashed border-gray-200 p-12 text-center">
          ...
          {filter === "all" && (
            <button
              onClick={() => setShowNew(true)}
              className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary-dark transition-all"
            >
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
