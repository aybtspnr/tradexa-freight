...
export function Admin() {
  ...
  async function loadData() {
    ...
    // Load subscriptions
    const { data: s } = await supabase.from("subscriptions").select("*").in("status", ["active", "trialing"]);
    setSubscriptions(s ?? []);
    ...
    setAgg({
      ...
      shippers: profilesData.filter((pr) => pr.role === "shipper").length,
      carriers: profilesData.filter((pr) => pr.role === "carrier").length,
      totalOrders: allOrders.length,
      activeOrders: allOrders.filter((o) => !["delivered", "cancelled"].includes(o.status as string)).length,
      revenue: allOrders
...
function HighlightsManager() {
  ...
  async function handleToggle(carrierId: string, currentlyFeatured: boolean) {
    ...
    const { error } = await (supabase.rpc as any)("toggle_carrier_featured", {
