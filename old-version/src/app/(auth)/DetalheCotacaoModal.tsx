...
export function DetalheCotacaoModal({ quotationId, userId, isCarrier, onClose, onUpdated }: Props) {
  ...
  async function loadCarrierCompliance(bidList: Bid[], ncm: NcmInfo, ncmClassificationId?: string) {
    ...
    const profileMap = new Map((profiles ?? []).map((p: any) => [p.id, p.name || p.email || "Transportadora"]));
    ...
    for (const cId of carrierIds) {
      ...
      const carrierCerts = new Set((certs ?? []).map((c: any) => c.certification_type));
  ...
  /* handleAutoRequest é chamado pelo componente AutoQuotes
  async function handleAutoRequest(carrierName: string, price: number) {
    const { data: carrier } = await supabase
      .from("profiles")
      .select("id")
      .eq("role", "carrier")
      .or(`name.eq.${carrierName},email.eq.${carrierName}`)
      .maybeSingle();

    if (!carrier) {
      alert("Transportadora não encontrada. Peça para ela dar um lance manualmente.");
      return;
    }

    const { error } = await supabase.from("quotation_bids").insert({
      quotation_id: quotationId,
      carrier_id: carrier.id,
      price,
      notes: "Cotação instantânea automática",
      status: "pending",
    });

    if (error) { alert("Erro: " + error.message); return; }

    await supabase.from("quotations").update({ status: "bidding" }).eq("id", quotationId).eq("status", "open");
    loadData();
  }
  */
